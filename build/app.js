 ...
 */
  ...
  angular.module('autocomplete-alt', [] )
    .directive('autocompleteAlt', ['$q', '$parse', '$http', '$sce', '$timeout', 'external', 
      function ($q, $parse, $http, $sce, $timeout, external) {
    // keyboard events
    var KEY_DW  = 40;
    var KEY_RT  = 39;
    var KEY_UP  = 38;
...
tagsInput.directive('tagsInput', ["$timeout","$document","$window","tagsInputConfig","tiUtil", function($timeout, $document, $window, tagsInputConfig, tiUtil) {
    ...
    return {
        ...
        link: function(scope, element, attrs, ngModelCtrl) {
            var hotkeys = [KEYS.enter, KEYS.comma, KEYS.space, KEYS.backspace, KEYS.deleteIt, KEYS.left, KEYS.right],
            ...
            events
                ...
                .on('input-keydown', function(event) {
                    ...
                    shouldRemove = (key === KEYS.backspace || key === KEYS.deleteIt) && tagList.selected;
...
var app = angular.module('gigwellForm', [ 'formly', 'formlyMaterial', 'autocomplete-alt', 'ngTagsInput' ] );
...
app.run(function(formlyConfig, formlyValidationMessages) {
	...
	formlyConfig.setType({
    	name : 'inputtags',
		templateUrl : '/src/resources/fields/inputtags.html',
		extends: 'select'
    });
});
...
app.controller('MainCtrl', function MainCtrl($scope, external) {
	...
	vm.loadingData = external.getModelForm().then(function(response) {
	...
	functions['artist'] = function($scope, external) {
		...
		if ($scope.model.artist) {
			$scope.to.loading = external.getArtists($scope.model.artist).then(function(response) {
				$scope.to.options = response.data.data[$scope.model.artist].artists;
				return response;
			});
		}
	}
